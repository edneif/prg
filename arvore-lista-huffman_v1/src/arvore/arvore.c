...
#ifdef DEBUG
		printf("Arvore adicoina esq filho: %d  Pai: %d \n", esq,
				vertice_get_id(vertice));
#endif
...
#ifdef DEBUG
		printf("Arvore adicoina dir filho: %d  Pai: %d \n", dir,
				vertice_get_id(vertice));
...
void dfs_pre_ordem_recusivo(vertice_t* no_vert, FILE* fp) {
	...
	if (vertice_get_esq(no_vert))
		...
		fprintf(fp, "\t\"%d-%c:%d\" -- \"%d-%c:%d\" [label = %d];\n",
				vertice_get_id(no_vert), vertice_get_id(no_vert),
				vertice_get_freq(no_vert),
				vertice_get_id(vertice_get_esq(no_vert)),
				vertice_get_id(vertice_get_esq(no_vert)),
				vertice_get_freq(vertice_get_esq(no_vert)), 0);

	if (vertice_get_dir(no_vert))
		fprintf(fp, "\t\"%d-%c:%d\" -- \"%d-%c:%d\" [label = %d];\n",
				vertice_get_id(no_vert), vertice_get_id(no_vert),
				vertice_get_freq(no_vert),
				vertice_get_id(vertice_get_dir(no_vert)),
				vertice_get_id(vertice_get_dir(no_vert)),
				vertice_get_freq(vertice_get_dir(no_vert)), 1);
